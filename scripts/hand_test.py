...
motorcmd_pub = rospy.Publisher("/roboy/pinky/middleware/MotorCommand", MotorCommand,
                               queue_size=1)
rate = rospy.Rate(100)
...
def move_fingers(right_hand=True, targets=None, finger = "INDEX"):
    ...
    targets_scaled = [i * 1000 for i in targets]
    rospy.loginfo("targets_scaled: " + str(targets_scaled))
    hand_msg.setpoint = targets_scaled  # multiply all values in the array with 1000
    print(hand_msg.setpoint)
...
if __name__ == '__main__':
    ...
    move_fingers_choreography(right_hand=False, intervals=100, ranges=[0, 1])
