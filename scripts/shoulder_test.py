...
def move_shoulder(left_or_right, axis=0, shoulder_range=None, samples=200):

    msg = JointState()

    # msg.name = ["shoulder_" + left_or_right + "_axis" + str(axis)]
    # targets = [np.linspace(shoulder_range[0], shoulder_range[1], samples)]
    msg.name = ["shoulder_" + left_or_right + "_axis0", "shoulder_" + left_or_right + "_axis1"]
    targets = [np.linspace(shoulder_range[0], shoulder_range[1], samples),
               np.linspace(shoulder_range[0], shoulder_range[1], samples)]
...
if __name__ == '__main__':
    ...
    if args.manual_test:
        move_shoulder(args.arm, args.axis, args.ranges, args.samples)

    rospy.loginfo("Move backward")
    rospy.sleep(5.0)
    move_shoulder(args.arm, args.axis, list(reversed(args.ranges)), args.samples)
    rospy.sleep(1.0)
