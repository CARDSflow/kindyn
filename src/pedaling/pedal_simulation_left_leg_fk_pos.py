...
OLD_X_OFFSET = 0.7163902600571725 + 0.23003546879794612  # [m]
OLD_Y_OFFSET = -0.010388552466272516 + 0.010388308199859624  # [m]
OLD_Z_OFFSET = 0.2164376942146126 - 0.20527069599791542  # [m]

NEW_X_OFFSET = -0.23003546880974085 + (-0.716390260045379) # [m]
NEW_Y_OFFSET = -0.010388308215364166 + (0.010388552481776845) # [m]
NEW_Z_OFFSET = -0.2052706960113988 + (0.21643769422809594) # [m]

DIFF_X_OFFSET = NEW_X_OFFSET - OLD_X_OFFSET
DIFF_Y_OFFSET = NEW_Y_OFFSET - OLD_Y_OFFSET
DIFF_Z_OFFSET = NEW_Z_OFFSET - OLD_Z_OFFSET

print("DIFF_X_OFFSET = ", DIFF_X_OFFSET)
print("DIFF_Y_OFFSET = ", DIFF_Y_OFFSET)
print("DIFF_Z_OFFSET = ", DIFF_Z_OFFSET)

BIKE_VELOCITY = 0.0
PEDAL_SINGLE_ROTATION_DURATION = 20.0  # [seconds]
...
def get_position_left_foot_with_coord_trans():
    pos = get_position_left_foot()
    pos[0] = pos[0] + DIFF_X_OFFSET
    pos[1] = pos[1] + DIFF_Z_OFFSET
    return pos
...
def publish_velocity(joint_name, next_joint_angle, current_joint_angle, end_time):
    ...
    timeNow = time.time()
    if end_time > timeNow:
        duration = end_time - timeNow
    else:
        duration = 0.01

    published_velocity = check_output_limits(published_velocity)
...
def control_pedaling():
    ...
    _currState = INIT
    _currTrajectoryPoint = get_position_left_foot_with_coord_trans()
    ...
    while _runFSM:
        ...
        if _currState == PEDAL:
            ...
            # Initialize state
            print("Initializing PEDAL state")
            ...
            _currTrajectoryPoint = get_position_left_foot_with_coord_trans()
            print("_currTrajectoryPoint = ", _currTrajectoryPoint)
            current_pedal_angle = evaluate_current_pedal_angle(_currTrajectoryPoint)
            ...
            for joint in _jointsList:
                ...
                actual_joint_angle = get_joint_angle(joint, evaluate_current_pedal_angle(get_position_left_foot_with_coord_trans()))
