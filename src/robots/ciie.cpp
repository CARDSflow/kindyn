...
class CIIE: public cardsflow::vrpuppet::Robot{
public:
    /**
     * Constructor
     ...
     */
    CIIE(string urdf, string cardsflow_xml){

        if (!ros::isInitialized()) {
            int argc = 0;
            char **argv = NULL;
            ros::init(argc, argv, "CIIE");
        }
    ...
    void write(){
        ...
        if(left_hand_available) {
            ...
            msg.request.target_frames = {"hand_left", "elbow_core_left"};
            msg.request.type = 1;
        ...
        if(right_hand_available) {
            ...
            msg.request.target_frames = {"hand_right", "elbow_core_right"};
            msg.request.type = 1;
            geometry_msgs::Pose pose;
