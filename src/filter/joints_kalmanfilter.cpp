...
namespace BFL
{
            ...
            number_of_joints_(number_of_joints)
    {
        ...
        ColumnVector measNoiseGps_Mu(number_of_joints_);  measNoiseGps_Mu = 0;
        SymmetricMatrix measNoiseGps_Cov(number_of_joints_);  measNoiseGps_Cov = 0;
        for (unsigned int i=1; i<=number_of_joints_; i++) measNoiseGps_Cov(i,i) = pow(10.0,2);
        Gaussian measurement_Uncertainty_GPS(measNoiseGps_Mu, measNoiseGps_Cov);
        MatrixWrapper::Matrix Hgps(number_of_joints_,number_of_joints_*2);  Hgps = 0;
        ...
        MatrixWrapper::SymmetricMatrix covar(number_of_joints_); covar = 0;
    ...
    {
        // set prior of filter
        ColumnVector prior_Mu(number_of_joints_*2);

        for(unsigned int i=1; i<=number_of_joints_; i++){
            prior_Mu(i) = prior(i-1);
        }

        SymmetricMatrix prior_Cov(number_of_joints_*2);
    ...
    bool BFL::KinDynEKF::update(double dt, const Eigen::VectorXd &model_action, const Eigen::VectorXd &meas_state)
    {
